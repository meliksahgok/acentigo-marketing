const features = [
  ...
  },
  {
    ...
    description: 'Online rezervasyon formu, müşteri bilgileri, yaş doğrulama ve kapasite kontrolü ile sorunsuz süreç.',
  },
  {
    icon: (
      <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
  ...
  },
  {
    ...
    description: 'Detaylı satış raporları, komisyon takibi, gelir-gider analizleri ve nakit kasa yönetimi.',
  },
  {
    icon: (
      <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    description: 'Müşteri kayıtları, rezervasyon geçmişi, iletişim bilgileri ve segmentasyonu ile ilişki yönetimi.',
  },
  {
    icon: (
      <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl relative z-10">
        <div className="text-center mb-16">
          <h2 className="text-3xl md:text-5xl font-bold text-white mb-6 tracking-tight">
            Kapsamlı <span className="text-primary">Özellikler</span>
          </h2>
          <p className="text-lg text-gray-400 max-w-2xl mx-auto">
            Acenteniz için ihtiyacınız olan tüm araçlar, modern arayüz ve güçlü altyapı ile tek platformda.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {features.map((feature, index) => (
            <div
              key={index}
              className="group p-6 rounded-2xl bg-white/5 border border-white/10 hover:bg-white/10 hover:border-primary/50 transition-all duration-300 hover:-translate-y-1"
            >
